...
// 判断当前时间是否在北京时间 00:30-08:30 之间
export function IsInTime(begin:{h:number,m:number},end:{h:number,m:number}) {
  // 获取当前时间
  const now = new Date();
  // 获取当前 UTC 时间的小时和分钟
  ...
  const beijingMinutes = utcMinutes;
  if (
    (beijingHours > begin.h || (beijingHours === 0 && beijingMinutes >= begin.m)) &&
    (beijingHours < end.h || (beijingHours === 8 && beijingMinutes <= end.m))
  ) {
    return true
  }
  return false
